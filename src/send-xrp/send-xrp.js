...
import renderVistiaLogo from "../helpers/render-vistia-logo";
  ...
  try {
    ...
    await getWalletDetails({ client }).then(
      ({ accountReserves, account_data }) => {
        availableBalanceElement.textContent = `Available Balance: ${
          dropsToXrp(account_data.Balance) - accountReserves
        } XRP`;
      }
    );
  } catch (error) {
...
client.on("transaction", (response) => {
  if (
    response.validated &&
    response.transaction.TransactionType === "Payment"
  ) {
...
submitTxBtn.addEventListener("click", async () => {
  try {
    ...
    const txResult =
      result?.meta?.TransactionResult || result?.engine_result || ""; // Response format: https://xrpl.org/transaction-results.html
